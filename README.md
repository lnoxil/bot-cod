...
- в описании поста поддерживаются теги кнопок: `{{btn:Текст|order|success|inline|🛒}}`, `{{btn:Get Support|support|secondary|inline}}`, `{{btn:Verify|url|secondary|bottom||https://site.com}}` — кнопка визуально отображается внутри текста и привязывается к действию бота/URL.
